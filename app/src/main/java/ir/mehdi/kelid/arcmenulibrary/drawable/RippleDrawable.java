package ir.mehdi.kelid.arcmenulibrary.drawable;
...
import ir.mehdi.kelid.arcmenulibrary.util.ColorUtil;
...
//        Log.v(RippleDrawable.class.getSimpleName(), "touch: " + event.getAction() + " " + mState);
	...
	public void start() {
		...
		resetAnimation();
		scheduleSelf(mUpdater, SystemClock.uptimeMillis() + Util.FRAME_DURATION);
	    invalidateSelf();  
	}
	...
	public static class Builder{
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
            ...
            type = Util.getType(a, R.styleable.RippleDrawable_rd_maxRippleRadius);
            if(type >= TypedValue.TYPE_FIRST_INT && type <= TypedValue.TYPE_LAST_INT)
                maxRippleRadius(a.getInteger(R.styleable.RippleDrawable_rd_maxRippleRadius, -1));
            else
			    maxRippleRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_maxRippleRadius, Util.dpToPx(48)));
			rippleColor(a.getColor(R.styleable.RippleDrawable_rd_rippleColor, Util.colorControlHighlight(context, 0)));
